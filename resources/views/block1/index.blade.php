...
<div class="progressBackground bg-elegant-color-dark">
    ...
    <nav class="blockPicker">
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            <a class="nav-item nav-link active" href="/block1" role="tab" aria-controls="nav-home" aria-selected="true">Semester 1</a>
            <a class="nav-item nav-link" style="color: white;" href="/block2" role="tab" aria-controls="nav-profile" aria-selected="false">Semester 2</a>
        </div>
    </nav>
        ...
        <div class="card-header">
            <div class="row">
                <div class="col-6 text-left mt-2">
                    <h4>Propedeuse Semester 1</h4>
                </div>
                <div class="col-6 text-right mt-1">
                    <a class="btn btn-success" href="{{ url('/block1/create') }}">New result</a>
                </div>
            </div>
        </div>
            ...
            <table class='table table-striped table-hover'>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $row->cijfer }}</td>
                        <td>{{ $row->ECwaarde }}</td>
                        <td><a style="float: left;" class="action-button btn btn-warning btn-sm text-white" href="{{ url('block1/' . $row->id . '/edit') }}">update</a>
                            <form method="post" action="{{ url('block1/' . $row->id) }}">
                                @method('delete')
                                @CSRF
